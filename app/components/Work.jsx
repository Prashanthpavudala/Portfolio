    ...
    >
      ...
      <div className="bg-white w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-5 flex flex-col gap-2 duration-500 group-hover:bottom-7">
        ...
        <div className="flex items-center justify-between">
          {/* ✅ If both URLs exist, show both buttons */}
          <div className="flex gap-2">
            {project.producerUrl && (
              <button
                onClick={() => window.open(project.producerUrl, "_blank")}
                className="border border-black rounded-md px-3 py-1 text-xs font-medium hover:bg-lime-300 transition"
              >
                Producer Repo
              </button>
            )}
            {project.consumerUrl && (
              <button
                onClick={() => window.open(project.consumerUrl, "_blank")}
                className="border border-black rounded-md px-3 py-1 text-xs font-medium hover:bg-lime-300 transition"
              >
                Consumer Repo
              </button>
          ...
          {/* Send Icon (still clickable if you want) */}
          <div
            onClick={() => {
